...
foreach($request_dept_instance as $key => $value){
if($request_dept_instance[$key]["hod_sansation"] == 1){
...
$approved_requests_number = count($approved_requests);
  ...
  ?>
 <h3 class="warning"><?php echo htmlentities($approved_requests_number);?></h3>
...
// $sql ="SELECT * FROM requests INNER JOIN user_request ON user_request.req_id = requests.req_id AND user_request.req_id IS NOT NULL INNER JOIN staffs_info ON staffs_info.stf_id = user_request.stf_id INNER JOIN departements ON departements.dept_id = staffs_info.dept_id INNER JOIN roles ON staffs_info.role_id = roles.role_id INNER JOIN destination ON requests.dest_id = destination.des_id INNER JOIN transiport ON transiport.trans_id = requests.trans_id ORDER BY requests.req_action_date;";
...
<td><?php if($request_dept_instance[$key]["hod_sansation"] !=1):
...
?></td>
 ...
 <input data-target="#Request-view-details" req-id="<?php echo htmlentities($request_dept_instance[$key]["req_id"]) ?>" style="margin: 0px ;padding: 3px;" type="button" class="btn btn-info btn-glow view-request-details" value="View" data-toggle="modal" > 
  </td>
    <td style="padding: 0px">
...
<div class="modal fade bd-example-modal-lg" id="request-form" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
  ...
  <script type="text/javascript">
         ...
         $('.view-request-details').click(function(){  
           ...
           var HOD_viewing_singele_request = "HOD_viewing_singele_request";
           $.ajax({  
                url:"scripts/staff-request-details.php",  
                method:"post",  
                data:{req_id:reqId, hod_id:hod_id , hod_viewing_sinle_req:HOD_viewing_singele_request},
