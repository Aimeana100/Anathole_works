...
$sql ="SELECT requests.req_id as req_id, staffs_info.stf_id as stf_id, staffs_info.stf_fname as stf_fname,staffs_info.stf_lname as stf_lname, roles.role_name as role_id, departements.dept_name as dept_name, requests.req_departure as req_departure, requests.req_return as req_return, destination.des_name as destination, user_request.hod_sansation as req_hod_sansation FROM requests INNER JOIN user_request ON user_request.req_id = requests.req_id AND user_request.req_id AND user_request.hod_sansation = 1 INNER JOIN staffs_info ON staffs_info.stf_id = user_request.stf_id INNER JOIN departements ON departements.dept_id = staffs_info.dept_id AND departements.dept_id = :departement INNER JOIN roles ON staffs_info.role_id = roles.role_id INNER JOIN destination ON requests.dest_id = destination.des_id INNER JOIN transiport ON transiport.trans_id = requests.trans_id;";
